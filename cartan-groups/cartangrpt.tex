...
The $\omega$ are linear homogeneous forms in $du_{q}$, whose coefficients are functions of $u_{q}$. These forms are called \emph{Pfaffian forms}.  We call the preceding equations \emph{the relative components of an infinitesimal displacement of a trihedral.}\index{Pfaffian form@\textsc{Pfaff}ian form}
...
\label{sec:4}\index{components!relative components of infinitesimal displacement of a moving frame}
...
\label{sec:7}\index{equality condition}
...
\theoremstyle{shape1}\index{structure theorem}
...
\label{sec:9}\index{arc!arc of a space curve}\index{curve!space curves}
...
\label{sec:10}\index{Frenet formulae@\textsc{Frenet} formulae}
...
\end{strthm}\index{structure theorem}
...
\begin{contactcon}\index{contact!contact condition}
...
\paragraph{Equality problem.}
\label{sec:21}\index{reduced equation!of a space curve}
...
\label{cha:theory-minim-curv}\index{curve!minimal curves}
...
\label{sec:cyclic-trihedrals}\index{cyclic trihedrals}
...
\label{sec:26}\index{equality condition}
...
We set $\omega_{21}=k\,d\sigma$. We call the quantity $k$ curvature,\index{curvature!curvature of a minimal curve} or fourth order invariant \footnote{The name invariant signifies that $k$ does not vary when the curve undergoes a displacement.}.

According to \eqref{eq:2.18} the value of the curvature is independent of the orientation. We obtain formulae analogous to the \emph{Frenet formulae} (\fsref{10}, p.~\pageref{sec:10}):\index{Frenet formulae@\textsc{Frenet} formulae}
\begin{equation}
  \label{eq:2.20}
  \left\{
    \begin{aligned}
...
\end{align*}
These formulae, called \emph{Weierstrass's formulae},\index{Weierstrass formulae@\textsc{Weierstrass} formulae} furnish the parametric representation of the most general minimal curve.
...
\label{sec:39}\index{reduced equation!of a minimal curve}
...
\label{sec:40}\index{pseudo-arc of a minimal curve}
...
\label{sec:41}\index{reduced equation!of a minimal curve}
...
\label{cha:study-real-ruled}\index{real ruled surfaces}
...
\label{cha:study-ruled-isotr}\index{isotropic surfaces}
...
\begin{equation}
  ...
  \left\{
    ...
    \end{aligned}
  \right.
\end{equation}\index{Frenet formulae@\textsc{Frenet} formulae}
...
\emph{Structure theorem}. Reasoning analogous to those in paragraph \fsref{29} allows us to deduce from the structure theorem stated in paragraph \fsref{25} (p.~\pageref{sec:25}) the following result:\index{structure theorem}
...
\label{sec:57}\index{Frenet formulae@\textsc{Frenet} formulae}
...
\paragraph{Transformations, their inverses and products.}\index{transformation!product}
\label{sec:58}\index{transformation}
...
Consider two transformations $T$ and $T'$ operating on the same domain $D$. Consider the point $\mathbf{M}'=T\mathbf{M}$, then the point $\mathbf{M}''=T'\mathbf{M}'$, and finally the transformation that moves $\mathbf{M}''$ to $\mathbf{M}$: $\mathbf{M}''=T'(T\mathbf{M})$. We call this transformation the \emph{product of the transformation $T$ with the transformation $T'$}. We denote it by the symbol $T'T$.\index{product of two transformations}

In general the two products $TT'$ and $T'T$ constitute different transformations. We say that the transformations $T$ and $T'$ \emph{commute} when $TT'=T'T$.\index{\index{transformation}!commuting}
...
\emph{The identity transformation} is the one that associate each point of $D$ to the point itself. We represent it by the symbol $1$. We evidently have\index{transformation!identity and inverse}
\[
...
\paragraph{Groups.}\index{transformation!group}
\label{sec:59}
...
By the \emph{subgroup}\index{subgroup} of a given group we mean every subset of transformations of this group that is itself a group.
...
\label{sec:60}\index{parameter!parameter space}
...
\end{align*}
We assume that $S$ possesses an inverse. We say that $\mathbf{R}_{0}$ is a ``\emph{absolute reference system}'' and $S\mathbf{R}_{0}$ is a ``\emph{relative reference system}''.\index{coordinates, relative} We will determine the equations of the transformation $T$ with respect to the relative reference system.
...
\emph{Transforming a transformation by another.}\index{transformation transformed by another transformation} Let $S$ and $T$ be two transformations operating on the same domain and let $S$ have an inverse. $STS^{-1}$ is called ``$T$ transformed by $S$''.
...
\label{sec:63}\index{frame!moving frame of a family of transformations}
...
\emph{The moving frame of a group.}\index{frame!moving frame of a group}
...
\label{sec:64}\index{group!linear group acting on one variable}
...
\label{sec:65}\index{group!affine group}
...
\label{sec:66}\index{group!projective group}
...
\paragraph{Infinitesimal transformations.}\index{infinitesimal transformation}
\label{sec:68}
...
\label{sec:70}\index{components!relative components of infinitesimal displacement of a moving frame}
...
\label{sec:74}\index{components!absolute components}
...
\label{sec:76}\index{equality condition}
...
\label{sec:77}\index{structure theorem}
...
T_{a}+S_{a}S_{1}^{-1}.
\]
Hence the following proposition: consider a family of transformations $S_{a}$ whose moving frame $\mathbf{R}_{a}$ possesses the components and let $S_{1}$ be an arbitrarily chosen particular transformation of $S_{a}$. The transformations $S_{a}S_{1}^{-1}$ constitute a group, at least if the structure theorem applies to the frame $\mathbf{R}_{a}$ without needing to enlarge its range of definition. The transformations $S_{a}S_{1}^{-1}$ belong to a connected and finite dimensional group and their images in the parameter space of the group constitute a part of this space having the same number of dimensions as the space itself: such a set of transformations will be called the \emph{kernel of the group}\index{kernel of group}. This set, containing the identity transformation for $S_{a}=S_{1}$, constitute the a certain neighbourhood of the identity transformation inside the group.
...
\label{sec:79}\index{group!parameter group}
...
\]
does not have such an integral variety unless the vector $(A,B,C)$ is always perpendicular to its rotation. Systems possessing this property are called \emph{completely integrable.}\index{completely integrable Pfaffian system@completely integrable \textsc{Pfaff}ian system}

\paragraph{Construction of a group from its infinitesimal transformations (first procedure).}
\label{sec:85}\index{Pfaffian system@\textsc{Pfaff}ian system}
...
\label{sec:similar-groups}\index{group!similar groups}
...
The preceding examples show the utility of the following definition: two transformations of a group $G$ are said to be \emph{homologous} when one is the transformed image of the other by a transformation of $G$.\index{homologous!transformations}\index{homologous!subgroups}
...
\paragraph{Definition of isomorphism.}\index{group!isomorphic groups}
\label{sec:90}\index{homomorphic groups}
...
\label{sec:91}\index{class!class of objects}
...
Two groups $G(\Omega_{1})$ and $G(\Omega_{2})$ can be similar: it is necessary and sufficient that there exists between the objects $\Omega_{1}$ and $\Omega_{2}$ a bijective correspondence such that two homologous objects remain homologous after being subjected to the same transformation of $G$: two classes possessing this property will be said to be \emph{equivalent classes}.\index{class!equivalent classes}
...
\label{sec:spott-objects-same}\index{class!identification of objects in a class}\index{identification of objects in a class}
...
\emph{Definition.} A group $G$ is said to \emph{operate transitively} on a class of objects $\omega$ when there always exists a transformation of this group transforming one arbitrary chosen object $\omega$ to another arbitrarily chosen object $\omega$.\index{group!transitive group}
...
A group $G(\omega)$ which is not transitive is called \emph{intransitive}.\index{group!intransitive group}
...
\label{sec:93}\index{orientation!of objects in a class}
...
Let $g_{2}$ be the subgroup leaving the object $\omega_{2}=S\omega_{1}$ invariant. $g_{2}=Sg_{1}S^{-1}$ is the transformed image of the subgroup $g_{1}$ by $S$. Hence \emph{two homologous bodies}\index{homologous!bodies}\index{body!homologous bodies} (which we understand as two bodies defined by two homologous subgroups) \emph{are equivalent.}
...
\label{sec:98}
The study of groups isomorphic to a given grou $G$ thus amounts to the determination of all the subgroups of $G$. We will learn in chapter \ref{cha:real-given-abstr} (\fsref{115}, p.~\pageref{sec:115})\index{group!abstract group} to what problems of differential and integral calculus we can reduce this determination to. This determination does not further necessitate the knowledge of the group $G$ itself, but only the knowledge of the way its operations compose, its ``law of composition'' \index{composition law of a group} \footnote{Such a law possesses the following characteristics:
...
Given a group $G$ operating on $n$ variables $x_{1}$, $x_{2}$, $\dots$, $x_{n}$, we agree to say that a group $G'$ operating on these $n$ variables $x_{i}$ and the $p$ other variables $y_{1}$, $y_{2}$, $\dots$, $y_{p}$ is a \emph{holoedric prolongation}\index{holoedric prolongation of a group} of $G$
\begin{enumerate}
...
\paragraph{Examples.}
\label{sec:102}
{\small\index{group!displacement group in the plane}
...
\emph{Group of homographic transformation}: $x'=\dfrac{ax+b}{a'x+b'}$, $ab'-ba'>0$.\index{group!homographic acting on one variable}
...
\paragraph{}
\label{sec:103}\index{group!intransitive group}
...
\label{sec:113}\index{defining equations of the transformations of a group}
...
\label{sec:116}\index{realisation of an abstract group}
...
\label{sec:119}\index{realisation of an abstract group}
...
Let us apply the following \emph{contact condition}:\index{contact!contact condition}
...
\label{sec:131}\index{frame!\textsc{Frenet} frame}
...
\paragraph{Introduction.}
\label{sec:133}\index{curve!plane curves in unimodular affine geometry}
...
\paragraph{Definition of elements of order $>3$.}
\label{sec:137}
The frames of orders $>3$ coincide with the Frenet frame. There exists an invariant of order $4$: $k=\dfrac{\omega_{21}}{d\sigma}$. We call it the \emph{affine curvature}.\index{curvature!curvature of an affine curve} There exists invariants of orders $P>4$, namely $\dfrac{d^{P-4}k}{d\sigma^{P-4}}$.
...
In other words, we have the \emph{Frenet formulae}\index{Frenet formulae@\textsc{Frenet} formulae}
\begin{equation}
  \label{eq:10.2.8}
  \left\{
    \begin{aligned}
...
\label{sec:139}\index{reduced equation!of a plane curve in affine geometry}
...
\label{sec:proj-geom-study}\index{curve!plane curves in projective geometry}
...
\]
$\omega_{01}$ is hence an invariant form of order $5$: we denote it by $d\sigma$ and we call $\sigma$ the \emph{projective arc}.\index{arc!projective arc}
...
\paragraph{Definition of elements of order $>6$.}
\label{sec:149}
There exists an invariant of order $7$: $k=-\dfrac{\omega_{10}}{d\sigma}$. We call it the \emph{projective curvature}.\index{curvature!curvature of a projective curve}

There exists an invariant of order $P>7$: $\dfrac{d^{P-7}k}{d\sigma^{P-7}}$.

The instantaneous displacement of the Frenet frame is characterised by the \emph{Frenet formulae}:\index{Frenet formulae@\textsc{Frenet} formulae}
\begin{equation}
  \label{eq:10.3.16}
  \left\{
    \begin{aligned}
...
\paragraph{Reduced equations.}
\label{sec:151}\index{reduced equation!of a plane curve in projective geometry}
...
\]
where $k$ is the projective curvature. The $y$ axis is the \emph{projective normal}\index{normal!projective normal}.
...
\]
We see that the second tangent at the double point is the projective normal and the straight line of the inflections is the third side $\mathbf{A}_{1}\mathbf{A}_{2}$ of the Frenet frame. The consideration of the bundle of cubics having at $\mathbf{A}$ a seventh order contact with the curve and of the \emph{Halphen point}\index{Halphen point@\textsc{Halphen} point} common to all these cubics allow us to characterise the projective curvature.
...
\label{cha:struct-equat-e}\index{structure constants}
...
\paragraph{}
\label{sec:152}\index{Darboux equations@\textsc{Darboux} equations}
...
The quantity $\omega'$ remains invariant when we change the choice of independent variables. We call $\omega'$ the \emph{exterior derivative} or the \emph{bilinear covariant} of $\omega$.\index{bilinear covariant}\index{exterior derivation}

\paragraph{Exterior products.}
\label{sec:156}\index{exterior product}
...
\textsc{Frobenius theorem.} \emph{Consider a $R$ dimensional space of coordinates $X_{1}$, $\dots$, $X_{R}$ and $N$ $(<R)$ linearly independent Pfaffian forms $\Pi_{1}(X,dX)$, $\dots$, $\Pi_{N}(X,dX)$. The necessary and sufficient condition for that the Pfaffian system}\index{Frobenius theorem@\textsc{Frobenius} theorem}
\begin{equation}
  \label{eq:11.33}
  \Pi_{1}(X,dX)=0,\qquad\dots,\qquad\Pi_{N}(X,dX)=0
\end{equation}
...
\label{sec:167}\index{group!in one parameter}
...
\label{sec:168}\index{group!in two parameters}
...
\paragraph{Introduction.}
\label{sec:173}\index{curve!plane curves in projective geometry}
...
\paragraph[{Projective development of a curve onto a conic.}]{Projective development of a curve onto a conic \footnote{For more details, \emph{see} \cite{20}, p.~51--69.}.}\index{projective development of a curve onto a conic}
\label{sec:183}
...
\label{sec:189}\index{equality condition}
...
\label{sec:191}
The lines $\omega_{2}=0$ and $\omega_{1}=0$ are characterised geometrically by the property that $d\vec{\mathbf{A}}$ and $d\ivec_{3}$ remain parallel  when $\mathbf{A}$ describes one of them: they are the curvature lines of the surface. The lines that annihilate $\omega_{1}\omega_{13}+\omega_{2}\omega_{23}$ are the asymptotic lines,\index{asymptotic lines} and $f_{,1}$ and $f_{2}$ are the ratios of increments of $f$ in the directions of the paths when we displace the respectively in the directions of the curvature lines.\index{curvature lines}
...
\paragraph{Reduced equation.}
\label{sec:194}\index{reduced equation!of a surface}
...
\label{sec:195}\index{deformation!deformation of surfaces}
...
\label{sec:196}\index{deformation!the general problem of deformation}
...
\label{sec:197}\index{exterior derivation}
...
\label{sec:199}\index{fundamental theorems of groups!third}
...
\label{sec:200}\index{canonical parameters}\index{parameter!canonical}
...
\label{sec:205}\index{infinitesimal transformation}\index{transformation transformed by another transformation}
...
\end{equation}\index{bracket of two infinitesimal transformations}
...
Three arbitrary infinitesimal transformations $\mathbf{X}f$, $\mathbf{Y}f$, $\mathbf{Z}f$ satisfy the \emph{Jacobi identity}\index{Jacobi identity@\textsc{Jacobi} identity}
\begin{equation}
  \label{eq:14.6}
  [\mathbf{X}(\mathbf{YZ})]+
  [\mathbf{Y}(\mathbf{ZX})]+
...
\label{sec:two-fund-theor}\index{fundamental theorems of groups!second (\textsc{S.~Lie})}
...
\label{sec:210}\index{Lie's structural equations@\textsc{Lie}'s structural equations}
...
\]
\footnotetext{The constants $c_{pqs}$ are the structural constants of the group.}
...
\label{sec:214}\index{group!acting on one variable}
...
\label{sec:adjoint-groups-third}\index{group!adjoint group}

\paragraph{Non-linear adjoint group.}
\label{sec:215}\index{adjoint group}
...
\label{sec:218}\index{fundamental theorems of groups!third}
